...
pub struct ViewportPostProcessOptions {
    pub outline_color: [f32; 3],
    pub outline_width: f32,
    pub bloom_params: [f32; 4],
}

pub struct ViewportRenderRequest<'a> {
    pub device: &'a wgpu::Device,
    pub queue: &'a wgpu::Queue,
    pub uniform: &'a CameraUniform,
    pub view: &'a wgpu::TextureView,
    pub camera: &'a Camera,
    pub reference_images: &'a ReferenceImageStore,
    pub viewport_size: (u32, u32),
    pub post_process: ViewportPostProcessOptions,
}

impl ViewportResources {
    pub fn render_to_view(&mut self, request: ViewportRenderRequest<'_>) {
        request
            .queue
            .write_buffer(&self.camera_buffer, 0, bytemuck::bytes_of(request.uniform));
        self.ensure_offscreen_texture(
            request.device,
            request.viewport_size.0,
            request.viewport_size.1,
        );
        self.write_blit_params(request.queue, request.viewport_size, request.post_process);
        ...
        let offscreen_view = self
            .offscreen_view
            .as_ref()
            .expect("offscreen viewport texture should exist before blitting");
        self.encode_scene_pass(&mut encoder, offscreen_view);
        self.encode_blit_pass(&mut encoder, request.view);
        self.reference_overlay
            .render(super::reference_overlay::ReferenceOverlayPass {
                device: request.device,
                queue: request.queue,
                encoder: &mut encoder,
                target_view: request.view,
                camera: request.camera,
                viewport_size: request.viewport_size,
                store: request.reference_images,
            });
        request.queue.submit(std::iter::once(encoder.finish()));
    }
    ...
    pub fn screenshot(
        ...
        width: u32,
        height: u32,
        post_process: ViewportPostProcessOptions,
    ) -> Vec<u8> {
        // Write camera uniform
        queue.write_buffer(&self.camera_buffer, 0, bytemuck::bytes_of(uniform));
        self.ensure_offscreen_texture(device, width, height);
        self.write_blit_params(queue, (width, height), post_process);

        // Create final presentation texture
        ...
        let offscreen_view = self
            .offscreen_view
            .as_ref()
            .expect("offscreen viewport texture should exist before screenshot blit");
        self.encode_scene_pass(&mut encoder, offscreen_view);
        self.encode_blit_pass(&mut encoder, &view);
    ...
    fn write_blit_params(
        ...
        let blit_data: [f32; 12] = [
            ...
            post_process.outline_color[0],
            post_process.outline_color[1],
            post_process.outline_color[2],
            post_process.outline_width,
            post_process.bloom_params[0],
            post_process.bloom_params[1],
            post_process.bloom_params[2],
            post_process.bloom_params[3],
        ];
        queue.write_buffer(
    ...
    fn encode_scene_pass(&self, encoder: &mut wgpu::CommandEncoder, view: &wgpu::TextureView) {
    ...
    fn encode_blit_pass(&self, encoder: &mut wgpu::CommandEncoder, view: &wgpu::TextureView) {
        let Some(ref blit_bind_group) = self.blit_bind_group else {
            return;
        };
