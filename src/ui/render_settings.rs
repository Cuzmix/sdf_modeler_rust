...
pub fn draw(ui: &mut egui::Ui, settings: &mut Settings, actions: &mut ActionSink) {
    ...
    chrome::panel_header(
        ui,
        "Render Settings",
        "Tune lighting, marching quality, post-processing, and sculpt-specific rendering behavior.",
    );
    ui.add_space(10.0);
    ...
    chrome::section_card(
        ui,
        "Presets",
        "Switch between performance profiles or reset the whole render stack.",
        |ui| {
            ui.horizontal_wrapped(|ui| {
                chrome::badge(ui, BadgeTone::Muted, "Profiles");
    ...
    // --- Shadows ---
    settings_section(ui, "Shadows", true, |ui| {
    ...
    // --- Ambient Occlusion ---
    settings_section(ui, "Ambient Occlusion", true, |ui| {
    ...
    // --- Raymarching ---
    settings_section(ui, "Raymarching", false, |ui| {
        labeled_slider_i32(
            ui,
            "Max Steps",
    ...
    // --- Lighting ---
    settings_section(ui, "Lighting", false, |ui| {
    ...
    // --- Environment ---
    settings_section(ui, "Environment", false, |ui| {
        ui.horizontal(|ui| {
            ui.selectable_value(
        ...
        ui.checkbox(&mut config.env_reflection_enabled, "Enable Specular IBL")
            .on_hover_text("Enable roughness-aware indirect specular from the active environment");
    ...
    settings_section(ui, "Subsurface Scattering", false, |ui| {
        ui.checkbox(&mut config.sss_enabled, "Enable SSS")
            .on_hover_text(
            "Thickness-based subsurface scattering (simulates light passing through thin geometry)",
        );
        ui.add_enabled_ui(config.sss_enabled, |ui| {
            labeled_slider(
                ui,
                "Strength",
                &mut config.sss_strength,
                0.5..=20.0,
                false,
    ...
    // --- Fog ---
    settings_section(ui, "Fog", false, |ui| {
    ...
    settings_section(ui, "Volumetric Scattering", false, |ui| {
    ...
    // --- Bloom ---
    settings_section(ui, "Bloom", false, |ui| {
    ...
    // --- Gamma / Tonemapping ---
    settings_section(ui, "Gamma", false, |ui| {
        labeled_slider(
            ui,
            "Gamma",
            &mut config.gamma,
    ...
    // --- Selection Outline ---
    settings_section(ui, "Selection Outline", false, |ui| {
        ui.horizontal(|ui| {
            ui.label("Color:");
    ...
    // --- Cross-Section ---
    settings_section(ui, "Visualization", false, |ui| {
        ui.horizontal_wrapped(|ui| {
            for mode in [
                ShadingMode::Full,
                ShadingMode::Solid,
    ...
    if config.shading_mode == ShadingMode::CrossSection {
        settings_section(ui, "Cross-Section", true, |ui| {
            ui.horizontal(|ui| {
                ui.label("Axis:");
    ...
    settings_section(ui, "Sculpt", false, |ui| {
        labeled_slider(
            ui,
            "Safety Border",
