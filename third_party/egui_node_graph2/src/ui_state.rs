...
#[cfg(feature = "persistence")]
fn _default_instance_id() -> u64 {
    0
}

fn next_graph_editor_instance_id() -> u64 {
    static NEXT_ID: OnceLock<Mutex<u64>> = OnceLock::new();
    let lock = NEXT_ID.get_or_init(|| Mutex::new(1));
    let mut guard = lock.lock().expect("graph editor instance id mutex poisoned");
    let id = *guard;
    *guard += 1;
    id
}
...
pub struct GraphEditorState<NodeData, DataType, ValueType, NodeTemplate, UserState> {
    ...
    #[cfg_attr(feature = "persistence", serde(skip, default = "_default_instance_id"))]
    pub instance_id: u64,
    ...
    GraphEditorState<NodeData, DataType, ValueType, NodeKind, UserState>
{
    pub fn new(default_zoom: f32) -> Self {
        ...
        state.instance_id = next_graph_editor_instance_id();
        state
    }
}
