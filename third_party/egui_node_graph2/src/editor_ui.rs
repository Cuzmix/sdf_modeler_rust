    ...
    CategoryType: CategoryTrait,
{
    ...
    fn draw_graph_editor_inside_zoom(
        ...
        if let Some(ref mut node_finder) = self.node_finder {
            let mut node_finder_area = Area::new(Id::new((self.instance_id, "node_finder"))).order(Order::Foreground);
    ...
    DataType: DataTypeTrait<UserState>,
{
    ...
    fn show_graph_node(
        ...
        let window_response = ui.interact(
            interaction_rect,
            Id::new((self.graph_instance_id, self.node_id, "window")),
        ...
        let (shape, outline) = {
            let rounding_radius = (4.0 * pan_zoom.zoom).round().clamp(0.0, 255.0) as u8;
            let rounding_radius_f32 = rounding_radius as f32;
            let rounding = CornerRadius::same(rounding_radius);
            ...
            let body_rect = Rect::from_min_size(
                outer_rect.min + vec2(0.0, titlebar_height - rounding_radius_f32),
