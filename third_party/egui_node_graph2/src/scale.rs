...
impl Scale for Margin {
    fn scale(&mut self, amount: f32) {
        self.left = ((self.left as f32) * amount).round() as i8;
        self.right = ((self.right as f32) * amount).round() as i8;
        self.top = ((self.top as f32) * amount).round() as i8;
        self.bottom = ((self.bottom as f32) * amount).round() as i8;
    }
}

impl Scale for CornerRadius {
    fn scale(&mut self, amount: f32) {
        self.ne = ((self.ne as f32) * amount).round().clamp(0.0, 255.0) as u8;
        self.nw = ((self.nw as f32) * amount).round().clamp(0.0, 255.0) as u8;
        self.se = ((self.se as f32) * amount).round().clamp(0.0, 255.0) as u8;
        self.sw = ((self.sw as f32) * amount).round().clamp(0.0, 255.0) as u8;
    }
}
...
impl Scale for Shadow {
    fn scale(&mut self, amount: f32) {
        self.spread = ((self.spread as f32) * amount.clamp(0.4, 1.))
            .round()
