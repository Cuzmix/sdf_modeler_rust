...
pub fn build(
    ...
    let skia_checkout_root = build
        .skia_source_dir
        .parent()
        .expect("Skia source directory must have a parent");

    if !offline {
        println!("Synchronizing Skia dependencies");
        let original_dir = env::current_dir().expect("current dir");
        env::set_current_dir(skia_checkout_root).expect("switch to Skia checkout root");
        #[cfg(feature = "binary-cache")]
        crate::build_support::binary_cache::resolve_dependencies();
        let sync_success = Command::new(python)
            ...
            .current_dir(skia_checkout_root)
        ...
        assert!(sync_success, "`skia/tools/git-sync-deps` failed");
    }
