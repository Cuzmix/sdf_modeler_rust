...
| --- | --- | --- | --- |
| Export workflow and export progress | `src/ui/export_dialog.rs`, `src/ui/export_progress.rs`, `src/app/egui_frontend.rs`, `src/app/ui_panels.rs` | Flutter has document persistence but not mesh export UX, progress, or error handling parity. | Add bridge-owned export snapshot/commands and a Flutter export surface. |
